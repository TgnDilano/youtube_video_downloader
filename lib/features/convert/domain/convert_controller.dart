...
class ConvertController extends ChangeNotifier {
  ...
  static bool isAudioSource(String path) => _audioExtensions.contains(
    p.extension(path).toLowerCase().replaceFirst('.', ''),
  );

  List<ConvertTask> get activeTasks => tasks.where((t) => t.isActive).toList();

  List<ConvertTask> get finishedTasks => tasks
      .where(
        (t) =>
    ...
    switch (targetId) {
      ...
      case 'mov':
        return hasVideo &&
            ...
            (!hasAudio || audioIn({'aac', 'mp3', 'pcm_s16le', 'pcm_s24le'}));
      case 'webm':
        return hasVideo &&
            videoIn({'vp9', 'vp8', 'av1'}) &&
  ...
  Future<void> _run(ConvertTask task) async {
    ...
    final args = <String>[
      ...
      if (canCopy) ...[
        if (isAudioTarget(task.targetId)) ...[
          ...
          '-map',
          '0:v:0',
          if (info['hasAudio'] == 'true') '-map',
          '0:a:0',
        ],
        '-c',
        'copy',
        if (task.targetId == 'mp4') '-movflags',
        '+faststart',
      ] else ...[
    ...
    log.add(
      '--- Starting convert: ${task.sourceName} → ${task.targetLabel} '
      '($mode) ---',
    );
    ...
    try {
      ...
      if (exitCode == 0) {
        ...
        if (task.stderrTail.isNotEmpty) {
          log.addAll(
            task.stderrTail.sublist(
              task.stderrTail.length > 6 ? task.stderrTail.length - 6 : 0,
            ),
          );
        }
      }
